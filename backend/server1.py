...
def update_encaregado(
    ...
    """
    Atualiza um encarregado na tabela 'encarregado' usando o ID.
    updates pode ser dict ou JSON string.
    """
    _consume_meta(sessionId, action, chatInput, toolCallId)
...
def delete_encarregado(
    ...
    """
    Remove um encarregado da tabela 'encarregado' pelo ID.
    """
    _consume_meta(sessionId, action, chatInput, toolCallId)
    ...
    cur = conn.cursor()
    try:
        sql = "DELETE FROM `encarregado` WHERE id = %s"
...
def delete_aluno_encarregado(
    ...
    """
    Remove um encarregado da tabela 'aluno_encarregado' pelo ID.
    """
    _consume_meta(sessionId, action, chatInput, toolCallId)
    ...
    cur = conn.cursor()
    try:
        sql = "DELETE FROM `aluno_encarregado` WHERE id = %s"
        cur.execute(sql, (encarregado_id,))
        conn.commit()
