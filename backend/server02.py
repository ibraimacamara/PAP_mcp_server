...
def validate(data: dict):
    ...
    required = [
        "nome", "data_nascimento", "contato", "bi",
        "email", "curso_id", "turma_id", "encarregado_id"
    ]
...
def insert_aluno(data: str) -> dict:
    ...
    conn = get_connection()
    if not conn:
        return {"success": False, "message": "Sem ligação à BD"}
    ...
    try:
        ...
        """, (
            ...
            payload.get("morada"),
            payload.get("genero"),
            payload.get("distrito"),
            payload.get("freguesia"),
            payload["curso_id"],
            payload["turma_id"],
            payload["encarregado_id"]
        ))
...
@mcp.tool
def list_alunos_inscritos_curso(curso_id: int) -> dict:
    """
    Lista alunos inscritos num curso específico.
    """
    conn = get_connection()
    if conn is None:
        return {
    ...
    try:
        query = """
            ...
            FROM aluno a
            INNER JOIN aluno_curso ac
                ON ac.numero_aluno = a.numero_aluno
            WHERE ac.curso_id = %s
        """

        cur.execute(query, (curso_id,))
        rows = cur.fetchall()
        ...
        return {
            "success": False,
            "message": "Erro ao listar alunos inscritos no curso",
            "error": str(e)
        }
